...
Groups all events by `url`, counts occurrences, and returns the top 10 URLs ordered by count descending.
...
Groups events by `referrer`, counts distinct visitors per referrer, and excludes rows where `referrer` is empty (direct / no referrer traffic).
...
SELECT
  ...
  AVG(json_extract(properties, '$.$val'))  AS value
FROM events
...
<Info>
Device classification is based on the `screen_width` value reported by `window.screen.width` in the visitor's browser at the time of the event — not User-Agent string parsing.
</Info>
